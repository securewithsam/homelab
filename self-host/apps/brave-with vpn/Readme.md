...
Isolated Brave browser instances exiting through different VPN geolocations for testing websites from various regions. Built with Gluetun (VPN container) and LinuxServer Brave (browser-in-browser via KasmVNC).
Why This Exists
...
Gluetun — lightweight VPN client with native Surfshark support (WireGuard)
Brave — full Brave browser streamed to your browser via KasmVNC, all traffic routed through Gluetun's VPN tunnel
Port Map
Port	Geo	VPN Exit
...
Available Surfshark locations can be found in the Gluetun Surfshark wiki.
Operations
Task	Command
...
Gluetun	VPN client container	MIT
Brave (LinuxServer)	Browser-in-browser	GPL-3.0
Surfshark	VPN provider (WireGuard)	Commercial subscription
